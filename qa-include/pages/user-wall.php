...
if (qa_clicked('dowallpost')) {
	...
	if (!strlen($inmessage)) {
		...
		qa_wall_add_post($loginuserid, qa_get_logged_in_handle(), qa_cookie_get(), $useraccount['userid'], $useraccount['handle'], $inmessage . qa_lang('profile/post_wall_amount_withdrawn'), '');		//Mel: 16/01/22. Add the withdrawal text at the end
		//qa_wall_add_post($loginuserid, qa_get_logged_in_handle(), qa_cookie_get(), $useraccount['userid'], $useraccount['handle'], $inmessage, '');

		//Mel: 16/01/22. Deduct the amount withdrawn from coins column in userpoints table 
...
$qa_content['message_list'] = array(
	...
	'form' => array(
		'tags' => 'name="wallpost" id="wallform" onsubmit="qa_submit_wall_post(dowallpost, false); return false;" method="post" action="' . qa_self_html() . '"',	//Mel: 15/01/22. To add form id and onsubmit attr
...
if ($start == 0) { // only allow posting on first page
	if ($wallposterrorhtml) {
		...
		$qa_content['message_list']['form']['fields'] = array(
			'message' => array(
				'tags' => 'name="message" id="message" type="number" placeholder="' . qa_lang('profile/wall_form_placeholder') . $maxwithdrawalamount . '" required',	//Mel: 15/02/22. Allow only numeric entry into form
				//'tags' => 'name="message" id="message"',
				'value' => qa_html(@$inmessage, false),
				'rows' => 1,		//Mel: 15/01/22. Reduce height
				//'rows' => 2,
				'error' => qa_html(@$errors['message']),
			),

			//Mel: 15/01/22. Add hidden field containing coins/points balance
			'balance' => array(
				'tags' => 'name="balance" type="hidden"',
				'value' => qa_db_get_coins_balance($loginuserid),
			),

			//Mel: 15/01/22. Add hidden field containing min coins balance
			'minbalance' => array(
				'tags' => 'name="minbalance" type="hidden"',
				'value' => qa_opt('points_base'),
			),

			//Mel: 15/01/22. Add hidden field containing wallet address
			'coins' => array(
		...
		$qa_content['message_list']['form']['buttons'] = array(
			'post' => array(
				...
				//'tags' => 'name="dowallpost" onclick="return qa_submit_wall_post(this, false);"',		//Mel: 15/01/22.
...
foreach ($usermessages as $message) {
	...
	//Mel: 16/01/22. To hide the withdrawal wall list from other users except for the user who owns the wall
