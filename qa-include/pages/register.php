...
if (qa_clicked('doregister')) {
	...
	if (qa_user_limits_remaining(QA_LIMIT_REGISTRATIONS)) {
		...
		$inwallet = qa_post_text('wallet');		//Mel: 15/01/22
		...
		if (!qa_check_form_security_code('register', qa_post_text('code'))) {
			...
			$errors = array_merge(
				...
				qa_handle_wallet_filter($inwallet)	//Mel: 15/01/22
			);
...
$qa_content['form'] = array(
	//Mel: 15/01/22. Mint token when submitting
	...
	'buttons' => array(
		...
		//Mel: 14/01/22. To get wallet address from Metamask, etc
		'connect' => array(
