...
function qa_show_waiting_after(elem, inside) {
	if (elem && !elem.qa_waiting_shown) {
		...
		if (w) {
			...
			//Mel: 16/01/22. Disable element like button to prevent clicking while waiting
			//elem.disabled = true;
...
function qa_hide_waiting(elem) {
	...
	if (c) {
		...
		//Mel: 16/01/22. Turn back on element after waiting is over
		//elem.disabled = false;
...
async function qa_submit_wall_post(elem, morelink) {
	...
	window.location.reload();		//Mel: 16/01/22. Refresh withdrawal page to get new balance after withdrawal.
